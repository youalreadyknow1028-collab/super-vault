# 🏗️ Daily Construction Report
## Peter Camilli & Sons
**Date:** Thursday, April 2, 2026
**Day:** Thursday
**Weather:** 🌧️ Rainy, 46°F, ENE 14mph — wet and raw
**Prepared by:** Nick Flanagan, Site Superintendent
**Distribution:** Bob Camilli
**Agent:** Supervisor (@super)

---

## 📊 PARA Classification
- **Category:** #para-projects
- **Path:** 1_Projects/Daily-Reports/
- **Priority:** #priority-high
- **Tags:** #super #construction #daily-report #yorktown-dpw #mamaroneck-lmc #mamaroneck-parks #para-projects #priority-high #bob-camilli

## 🕸️ Ontology Links
- [[Nick Flanagan]] OVERSEES [[Yorktown-DPW]]
- [[Nick Flanagan]] OVERSEES [[Mamaroneck-LMC]]
- [[Nick Flanagan]] OVERSEES [[Mamaroneck-Parks]]
- [[Bob-Camilli]] OWNS [[Peter Camilli & Sons]]
- [[Supervisor]] GENERATES daily_report FOR [[Bob-Camilli]]
- [[Billy-Cerutti]] IS_CUSTODIAN_AT [[Ardsley-High-School]]
- [[Tumen-Signage]] INSTALLS_SIGNAGE_AT [[Ardsley-High-School]]
- [[Timmy]] PERFORMS_LABOR_AT [[Yorktown-DPW]]
- [[Andrew-Luzzi]] PERFORMS_EIFS_AT [[Yorktown-DPW]]
- [[Katarina-Legener]] PERFORMS_SURVEY_AT [[Mamaroneck-Parks]]

---

## 🎯 Executive Summary

| Project | Status | Today |
|---------|--------|-------|
| 🟢 **Yorktown DPW** | Active Construction | Timmy re-digging weeping pit. AWS stucco cleared — Bob approved color, Nick to deliver sample for records. Northeast Sprinkler confirmed April 7. |
| 🟢 **Mamaroneck Parks** | Crew Mobilizing Tomorrow | Bob delivered full plans to Katarina Legener ✅. Tectonic crew 7am Friday — CRITICAL. |
| 🟡 **Mamaroneck LMC** | Near Closeout | Shop vac and ladders still on site. Daikin startup unconfirmed. Richie Puleo shelves CO pending. |
| 🟡 **Ardsley Signs** | Scope Issue | Got building access ✅. Tumen on site but MISSING 7 Type C + 9 Type WV signs. Conference call with Dorothy — she will provide revised schedule. |

**Overall Status:** 🟢 Yorktown progressing but stucco blocked on your approval. Parks ready for Friday mobilization. Ardsley has a scope issue — Tumen missing signage. LMC still needs punchlist and equipment confirmation.

---

## 📍 Project Updates

### 1. 🏗️ Yorktown DPW — 281 Underhill Avenue
**Status:** 🔨 Active Construction

#### Today's Accomplishments

| Trade | Company | Work Performed | Status |
|-------|---------|----------------|--------|
| **Sitework** | Timmy | Re-digging water spreader / weeping pit to David Tetro dimensions | 🟡 IN PROGRESS |
| **Sitework** | Timmy | Went to Lowe's 9am — picked up gravel + pipe insulation | ✅ DONE |
| **EIFS** | Advanced Wall Systems | Stucco STALLED — waiting on Bob color approval | 🔴 BLOCKED |
| **Fire Suppression** | Northeast Sprinkler | Date CONFIRMED — Tuesday April 7th | ✅ CONFIRMED |

#### ✅ Stucco Color — Approved

> **Bob has approved the stucco color.**
>
> Nick just needs to deliver the Andrew Luzzi color sample to Bob for his records.

#### 🔴 CRITICAL — Door Opening Must Be Restricted to 90°

> The exterior door currently opens 180°. When Andrew installs the stucco base and base board, the door will hit.
>
> **This must be resolved before AWS can proceed — otherwise the door will be blocked by the finished stucco work.**

#### Pending Items

| Item | Status |
|------|--------|
| Stucco color approval from Bob | 🔴 YOUR DECISION NEEDED — blocks all stucco work |
| Restrict door to 90° | 🔴 MUST DO before stucco starts |
| Contact ABM Mechanical (ERV ductwork + condensate drain) | 🔴 NOT YET CONTACTED |
| Northeast Sprinkler on site | ✅ CONFIRMED — Tuesday April 7th |
| Copper pipe insulation (bunk room kitchen, exterior wall) | 🟡 Materials on site |
| Fix bottom 4" sheetrock on kitchen wall (bunk room) | 🟡 Pending |
| Weeping pit / level spreader | 🟡 Timmy re-digging today |

#### Blockers Decided Today

| Item | Decision |
|------|----------|
| Northeast Sprinkler date | ✅ **CONFIRMED — Tuesday April 7th** |

---

### 2. 🏗️ Mamaroneck Parks — 150 Myrtle Avenue
**Status:** 🟢 Survey Complete — Tectonic Mobilizing Tomorrow 7 AM

#### Today

| Item | Status |
|------|--------|
| **Full plans delivered to Katarina Legener** | ✅ CONFIRMED — Bob delivered |
| **Tectonic crew Friday 7am** | 🔴 CRITICAL — tomorrow morning |

#### 📅 Upcoming

| Date | Event | Status |
|------|-------|--------|
| **April 3 (Fri, 7 AM)** | Tectonic crew — elevations, stake building corners + retaining walls | 🔴 CRITICAL — TOMORROW |
| **April 9 (TBD)** | Next Construction Progress Meeting | 📅 TBD |

---

### 3. 🏗️ Mamaroneck LMC — 740 Boston Post Rd
**Status:** 🟡 Near Closeout

No site visit today.

#### 🔴 Blockers for Closeout

| Blocker | Status |
|---------|--------|
| **Shelves / cabinet doors CO** | 🔴 WAITING FOR RICHIE PULEO DATE |
| **Shop vac + ladders on site** | 🔴 STILL ON SITE — needs removal |
| **Daikin HVAC startup** | 🔴 STILL UNCONFIRMED — was supposed to be Fri/Mon (March 27/30) |

#### Pending Items

| Item | Action Needed |
|------|--------------|
| Richie Puleo date | Get date for shelves + cabinet doors change order |
| Shop vac + ladders | Remove from site |
| HVAC startup | Confirm if Buchanan Mechanical + Daikin made it to power up rooftop units |

---

### 4. 🏗️ Ardsley Signs — 300 Farm Road, Ardsley, NY
**Status:** 🟡 Scope Issue — Missing Signage

#### Today's Activity

> **Work from last summer's ARDSLEY UFSD - SCIENCE ROOM RENOVATION completed today.**

| Item | Status |
|------|--------|
| **Building access** | ✅ Billy Cerutti gave access — classrooms unlocked |
| **Oliver (Tumen) on site** | ✅ On site |
| **Plans / drawings** | 🔴 Oliver had NO plans — Nick got approved shop drawings from Bob |
| **Sign locations marked** | ✅ Nick marked all locations for Oliver |
| **Signs installed** | 🔴 Tumen did NOT bring all signage |
| **Conference call** | ✅ Nick + Bob + Dorothy (Tumen) — Dorothy informed of missing items |

#### 🔴 SCOPE ISSUE — Missing Signage

| Sign Type | Quantity Missing |
|-----------|-----------------|
| Type C signs | 7 |
| Type WV signs | 9 |

**Dorothy from Tumen Signage** participated in a conference call with Bob Camilli and Nick today. Dorothy was informed of the missing signage and will provide a revised installation date. Installation must be scheduled around school hours.

#### Note — Emergency Evacuation Plan Signs

> Shop drawings indicate emergency evacuation plan signs have been **removed from scope** (per cost engineering). This was confirmed on today's call with Dorothy.

#### Pending

| Item | Status |
|------|--------|
| Revised installation date from Dorothy (Tumen) | 🔴 FOLLOW UP — Dorothy to contact |
| Install remaining 7 Type C + 9 Type WV signs | 🔴 PENDING — awaiting revised schedule |

---

## 📋 Change Orders Summary

| CO | Project | Description | Status |
|----|---------|-------------|--------|
| ✅ **CO-Y-W8** | Yorktown DPW | W8 shaft wall assembly | **COMPLETE** |
| ✅ **CO-Y-Bump-Out** | Yorktown DPW | Bump out around heater pipe | **COMPLETE** |
| 🔴 **CO-LMC-001** | Mamaroneck LMC | Shelves + cabinet doors | **WAITING FOR RICHIE PULEO DATE** |
| ✅ **CO-P-001** | Mamaroneck Parks | Door hardware direction | **APPROVED** |
| ✅ **CO-P-002** | Mamaroneck Parks | Toilet wall relocation | **APPROVED** |

---

## 📧 Coordination Log

| With | Topic | Outcome |
|------|-------|---------|
| [[Bob-Camilli]] | Ardsley signage | Provided approved shop drawings; conference call with Dorothy (Tumen) |
| [[Billy-Cerutti]] | Ardsley access | ✅ Got building access — classrooms unlocked |
| [[Dorothy-Tumen]] | Missing signage | Informed — will provide revised installation schedule |
| [[Bob-Camilli]] | Stucco color | Sample ready — awaiting your approval |
| [[Northeast Sprinkler]] | Installation date | ✅ CONFIRMED — Tuesday April 7th |

---

## 🔴 Blockers / Decisions Needed TODAY

| Item | Owner | Urgency |
|------|-------|---------|
| **Deliver stucco sample to Bob for records** | Nick | 🟡 **TODAY** |
| **🔴 Restrict door to 90°** | Nick / TBD | 🔴 **CRITICAL — door will hit stucco** |
| Dorothy (Tumen) revised schedule | Dorothy | 🔴 HIGH — missing 7 Type C + 9 Type WV signs |
| ABM Mechanical contact (ERV + condensate) | Nick | 🔴 HIGH |
| Richie Puleo — shelves/doors date | Richie Puleo | 🔴 HIGH |
| Shop vac + ladders removal | Nick | 🔴 HIGH |
| Daikin startup confirmation | Nick | 🟡 MEDIUM |

---

## 🏗️ Manpower Today

| Trade | Company | Count |
|-------|---------|-------|
| Signage | Tumen Signage (Oliver) | 1 |
| Sitework | Timmy | 1 |
| Site Super | Nick | 1 |

---

## 🌧️ Weather Impact

Rainy, 46°F, ENE 14mph — wet and raw conditions. Limited exterior work possible. Friday looks clear (39–55°F) — good weather window for exterior stucco and Tectonic survey.

**Saturday:** 🌧️ Rain returning.

---

## 📅 Tomorrow's Plan (April 3)

- **7:00 AM — TECTONIC CREW AT MAMARONECK PARKS** — elevations, stake building corners + retaining walls (CRITICAL)
- Deliver stucco sample to Bob for his records
- Check Timmy's weeping pit — verify proper dimensions per David Tetro
- AWS stucco work — color ALREADY APPROVED, ready to proceed
- Restrict door to 90° before stucco work begins
- Follow up with Dorothy (Tumen) on revised sign installation date
- Install copper pipe insulation (materials on site)
- Fix bottom 4" of sheetrock on kitchen wall in bunk room

---

## 📋 Week Ahead

| Date | Project | Event |
|------|---------|-------|
| **April 3 (Fri, 7 AM)** | Mamaroneck Parks | Tectonic crew — CRITICAL |
| **April 7 (Tue)** | Yorktown DPW | Northeast Sprinkler on site |
| **April 9 (TBD)** | Mamaroneck Parks | Next Construction Progress Meeting |

---

**Report generated by Supervisor (@super)**
**📧 Sent via SendClaw — super_nick@sendclaw.com**
**⏰ Generated:** 2026-04-02

## HASHTAGS
#super #construction #daily-report #para-projects #priority-high #yorktown #mamaroneck-parks #mamaroneck-lmc #ardsley #tumen
